* Install requirments.txt
* Run main.py
    * This will run the following:
        1- evaluate_optimal_classifiers
            - Runs the optimal hyperparameters for all dataset/classifier combinations
            - Plots results and stores the plots into a .png file
        2- run_experiments_classifiers
            - Runs the different experiments for each algoruthm and dataset
        3- get_optimal_classifier_hyper_parameters
            - Gets the optimal hyperparameters for each dataset/classifier combo using grid search
