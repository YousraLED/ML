...
class LDA(BaseClassifier):
    ...
    def _set_classifier(self, parameters):
        self.classifier = LinearDiscriminantAnalysis(tol=1e-4)
    ...
    def run_experiments(self, parameter_groups, dataset_name, dataset_algorithm_settings=None, cv=5):
        ...
        formatted_params = {'classifier__n_components': 1}
    ...
    def _plot_training_learning_curve(self, mean_test_scores,
                                      ...
                                      train_sizes,
                                      dataset_name):
        title = f"Learning Curve - {self.classifier_name} - Dataset: {dataset_name}"
    ...
    def _plot_training_time_versus_samples(self, mean_train_times,
        ...
        title = f"Training Time - {self.classifier_name} - Dataset: {dataset_name}"
        fig, ax = plt.subplots(nrows=1, ncols=1)
    ...
    def _plot_training_time_versus_score(self, mean_train_times,
        ...
        title = f"Model Performance - {self.classifier_name} - Dataset: {dataset_name}"
        fig, ax = plt.subplots(nrows=1, ncols=1)
